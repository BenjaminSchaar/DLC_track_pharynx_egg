...
def main(arg_list=None):
    ...
    parser.add_argument('--arena_y', help='arena Y dimension (default: 40.5mm in wbfm)', required=False,
                        default='40.5mm')
    ...
    if has_odor_data and x_odor is not None and y_odor is not None:
        ...
        df_worm_parameter[f'distance_to_odor_{skel_pos_0}'] = df_worm_parameter[
            f'distance_to_odor_{skel_pos_0}'].astype(
            float)
    ...
    df_combined = pd.concatimport pandas as pd
import argparse
import os
import numpy as np
import yaml
